 ...
 */
export function hideOfficialTurnNavigation(
  doc: Document,
  scheduler: NavHideScheduler = browserScheduler(),
): () => void {
  ...
  const apply = () => {
    ...
    for (const element of doc.querySelectorAll(OFFICIAL_NAV_SELECTORS)) {
      if (element.hasAttribute(OFFICIAL_NAV_HIDDEN_ATTRIBUTE)) continue
 ...
 */
function browserScheduler(): NavHideScheduler {
